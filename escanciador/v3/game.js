...
const sound = {
  ...
  unlock() {
    try {
      ...
      return Promise.resolve(resumed).then(()=>this.report()).catch(()=>{this.report();return false;});
    } catch(error) {
  ...
  pour(active,inside) {
    ...
    this.gain.gain.setTargetAtTime(active?(inside ? .09 : .06):0,t,.035);
