...
const ui = Object.fromEntries(['remaining','pour-state','caught','spilled','result','final-caught','final-spilled','again','hint','audio-test','quiz','summary','question','verdict','answer-detail','quip','answer-source','option-0','option-1','option-2','option-3'].map(id => [id, document.getElementById(id)]));
...
const sound = {
  context:null, gain:null, filter:null, source:null, buffer:null, lastSplash:-1,
  ...
  sync() {
    ...
    if(!this.active||c.state!=='running') {
      this.stopSource();
...
const QUESTIONS = [
  {text:'¿Cuál es la capital de Asturias?',options:['Gijón','Oviedo','Avilés','Mieres'],answer:1,fact:'La capital de Asturias es Oviedo.',source:'https://www.sidraturismoasturias.es/entidades/ayto-oviedo/'},
  {text:'¿Qué día se celebra el Día de Asturias?',options:['8 de septiembre','25 de julio','12 de octubre','1 de mayo'],answer:0,fact:'El Día de Asturias se celebra el 8 de septiembre.',source:'https://sidradeasturias.es/dia-de-asturias/'},
  {text:'¿Qué legumbre es la protagonista de la fabada?',options:['Lentejas','Garbanzos','Guisantes','Fabes'],answer:3,fact:'La fabada se prepara con fabes, acompañadas del compango.',source:'https://www.turismoasturias.es/en/gastronomia/fabada/-/asset_publisher/MTK2sQFT6y6u/content/id/616787'},
  {text:'¿Qué calle de Oviedo se conoce como el Bulevar de la Sidra?',options:['Uría','Gascona','Corrida','Marqués de San Esteban'],answer:1,fact:'Gascona es el Bulevar de la Sidra de Oviedo.',source:'https://www.turismoasturias.es/en/descubre/fiestas-de-interes-turistico/la-preba-de-la-sidra'},
  {text:'Si pides oricios en un chigre, ¿qué vas a comer?',options:['Mejillones','Berberechos','Erizos de mar','Calamares'],answer:2,fact:'Los oricios son erizos de mar.',source:'https://www.turismoasturias.es/gastronomia/sidra'},
  {text:'¿Cómo se llama la forma tradicional de servir la sidra desde lo alto?',options:['Decantar','Escanciar','Macerar','Destilar'],answer:1,fact:'Escanciar es servir la sidra desde lo alto para que golpee el vaso.',source:'https://www.turismoasturias.es/gastronomia/sidra'}
];
...
function reset() {
  state = { glassX: 290, time: 0, emitted: 0, caught: 0, spilled: 0, drops: [], splashes: [], holding: false, pointer: null, offset: 0, started: false, done: false, question:null, answered:false, keys: new Set(), flash: 0 };
  sound.pour(false,false); accumulator = 0; ui.result.hidden = true; ui.quiz.hidden=true;ui.summary.hidden=true; updateUI(); draw();
}
...
function step(dt) {
  ...
  if(pouring) {
    ...
    while(accumulator >= 1 && state.emitted < total) {
      ...
      state.drops.push({x:m.x,y:m.y,vx:165+(p.angle-.3)*220,vy:125,judged:false});
    }
  }
